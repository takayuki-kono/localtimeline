...
補足仕様は `docs/focus_timeline.md` を参照。
...
## 使い方
- **手動でFocus timeline生成 + Sheets同期**: `analyze.bat` をダブルクリック。
  - `focus_log.csv` にデータがあるのに未処理の過去日があれば、`focus_timeline_YYYY-MM-DD.png` の生成と Weighted Focus Time の Sheets 記入をまとめて行います（完了済み日付は `focus_output_state.json` に記録します）。
...
## Focus timeline出力とSheets記入の「完了日付」管理
- `focus_log.csv` にデータが存在する全日付について、未完了分のみまとめて処理します。
  - **focus_timeline画像生成**
  - **Weighted Focus Time を Google Sheets に記入**
- 完了した日付は `focus_output_state.json` に記録され、次回以降はスキップされます。
