...
- **未処理分の自動補完（バックフィル）**: `analyze.bat` 実行時、`focus_log.csv` にデータがあるのに未処理の過去日があれば、`focus_timeline_YYYY-MM-DD.png` の生成と Weighted Focus Time の Sheets 記入をまとめて行います（完了済み日付は `focus_output_state.json` に記録します）。
