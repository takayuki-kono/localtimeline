...
- **Break**: 共通色（Teal）。バー高さは常に最大（100%）
- **Focus**: `task` 名ごとに **ベース色を決定**する（色の濃さは `score` に依存しない）
  - `task` が空/欠損の Focus は `(No Task)` としてまとめる
  - 色はタスク名から決定される（同じタスク名なら日付が変わっても同じ色になる）

### 集中度（`score`）とバー高さ

- Focus バーの **高さ** に `score`（1–10）を反映する。未設定は 5 として扱う
- 高さ比率 = `score / 10`（例: `score=2` → 最大高さの 20%、`score=10` → 100%）
- バーは下揃えで描画する（低い集中度ほど低いバーになる）
...
以下は **リポジトリ直下** に置くこと（`analyze.bat` が相対パスで参照する）:

- `run_analyze.bat`
- `process_focus_outputs.py`
- `generate_focus_timeline.py`
- `focus_log.csv`（入力データ）
- `focus_output_state.json`（処理済み日付の記録）
