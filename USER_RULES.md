...
5. **Author設定（自認モデルに基づく）**: git commit 提示時は必ず `--author` フラグを使用する。
   - **自認モデル**: エージェント（AI）が自認するモデル・実行環境に基づいて author の**具体的な値**を提示すること。例: Cursor 上ならそのエージェントのモデル識別子（例: `Auto <auto@cursor.com>`）、Gemini/Claude 等ならそのモデルに合わせる。
   - **プレースホルダー禁止**: `Your Name <your.email@example.com>` 等の仮の値は用いない。必ず自認モデルに基づく実名・実値で提示する。
...
4. **Git Commit (JAPANESE ONLY)**:
   ...
   - Use `--author` flag based on the agent's self-identified model (e.g. in Cursor, the current model identity).
